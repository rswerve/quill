...
export function buildEditProtocolLines(): string[] {
  return [
    ...
    "- A TEXT or FORMAT edit cannot merge, split, add, or remove separate PARAGRAPHS, list items, or headings (blocks separated by a blank line in the Markdown source). Changing one existing block's TYPE between a heading and a normal paragraph IS possible with a structural edit (below); but joining, splitting, adding, or deleting whole blocks, or converting to or from a list, still needs Editing mode — emit no edit for those and explain in prose.",
    ...
    '- To change one existing block\'s TYPE, use a "structural" edit: {"find":"<exact reading-text substring>","structural":{"to":"paragraph"}} turns the heading that CONTAINS that text into a normal paragraph, and {"find":"<exact substring>","structural":{"to":"heading","level":2}} turns the paragraph that contains it into a level-2 heading. "find" is plain reading text that UNIQUELY IDENTIFIES ONE block — it need not be the block\'s whole text; "level" (1–6) is REQUIRED when "to" is "heading" and must be omitted otherwise. A structural edit carries NEITHER "replace" NOR "format".',
    "- Structural edits currently support ONLY converting an existing heading to a paragraph or a paragraph to a heading. Creating, deleting, merging, or splitting blocks, converting to or from a list (bulleted, numbered, or checklist), and changing an existing heading's level are NOT available yet — for any of those, emit no edit and explain in prose that it needs Editing mode.",
