...
export function buildEditProtocolLines(): string[] {
  return [
    ...
    "- A TEXT or FORMAT edit cannot merge, split, add, or remove separate PARAGRAPHS, list items, or headings (blocks separated by a blank line in the Markdown source). Changing one existing block's TYPE — a heading to/from a paragraph, or a SINGLE-item list to/from a paragraph — and SPLITTING a paragraph into several paragraphs ARE possible with a structural edit (below); but merging, adding, or deleting whole blocks still needs Editing mode — emit no edit for those and explain in prose.",
    ...
    '- To change one existing block\'s TYPE, use a "structural" edit whose "find" is plain reading text that UNIQUELY IDENTIFIES ONE block (it need not be the block\'s whole text) and which carries NEITHER "replace" NOR "format". The "to" value picks the new type: "paragraph" turns the heading — or the single-item list — that contains the text into a normal paragraph; "heading" turns a paragraph into a heading and REQUIRES a "level" of 1–6 (omit "level" for every other target); "bulletList", "orderedList", or "taskList" turns the paragraph that contains the text into a single-item list of that kind. Examples: {"find":"Section title","structural":{"to":"paragraph"}}, {"find":"Make this a heading","structural":{"to":"heading","level":2}}, {"find":"A standalone point","structural":{"to":"bulletList"}}.',
    '- To SPLIT a paragraph into several paragraphs, use a "structural" edit with "split" (an array of 2+ pieces) INSTEAD of "to": each piece must be that paragraph\'s OWN reading text, split only at the spaces — no added, removed, or changed words, and no "level". Example: {"find":"First point. Second point.","structural":{"split":["First point.","Second point."]}}. A structural edit carries EXACTLY ONE of "to" or "split".',
    "- Structural edits support heading↔paragraph, SINGLE-item list↔paragraph, and splitting a paragraph into paragraphs. Converting a list of MORE THAN ONE item, changing a list's kind (bulleted↔numbered↔checklist), changing an existing heading's level, and creating, deleting, or merging blocks are NOT available yet — for any of those, emit no edit and explain in prose that it needs Editing mode.",
