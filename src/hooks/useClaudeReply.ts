...
export function buildEditProtocolLines(): string[] {
  return [
    ...
    '- A TEXT edit ("find"+"replace") must start and end inside ONE paragraph, list item, or heading, and its "replace" must be single-block prose (no newlines). Quill cannot yet merge, split, or add blocks as a tracked suggestion — when asked for such a structural change (e.g. turning bullets into a paragraph), emit no edit for it and explain in prose that it needs Editing mode.',
    '- A FORMAT edit\'s "find" MAY span multiple paragraphs, list items, or headings: separate each block\'s text with a SINGLE "\\n" — never a blank line. The Markdown source shows blank lines between paragraphs, but the text you match against joins blocks with one newline.',
