...
export function buildEditProtocolLines(): string[] {
  return [
    ...
    "- Structural edits support heading↔paragraph, list↔paragraph (a flat list of ANY number of items, each item a single line, converts to one paragraph), and splitting a paragraph into paragraphs. A list whose items are nested or hold more than one block, changing a list's kind (bulleted↔numbered↔checklist), changing an existing heading's level, and creating, deleting, or merging blocks are NOT available yet — for any of those, emit no edit and explain in prose that it needs Editing mode.",
