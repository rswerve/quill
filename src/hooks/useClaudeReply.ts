...
export function buildEditProtocolLines(): string[] {
  return [
    ...
    "- A TEXT or FORMAT edit cannot merge, split, add, or remove separate PARAGRAPHS, list items, or headings (blocks separated by a blank line in the Markdown source). Changing one existing block's TYPE — a heading to/from a paragraph, or a SINGLE-item list to/from a paragraph — IS possible with a structural edit (below); but joining, splitting, adding, or deleting whole blocks still needs Editing mode — emit no edit for those and explain in prose.",
    ...
    "- Structural edits support heading↔paragraph and SINGLE-item list↔paragraph only. Converting a list of MORE THAN ONE item, changing a list's kind (bulleted↔numbered↔checklist), changing an existing heading's level, and creating, deleting, merging, or splitting blocks are NOT available yet — for any of those, emit no edit and explain in prose that it needs Editing mode.",
