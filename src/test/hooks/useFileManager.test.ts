...
describe('useFileManager', () => {
  ...
  describe('saveFile', () => {
    it('returns null and does not invoke when no filePath is set', async () => {
      ...
      let res: string | null;
      await act(async () => {
        res = await result.current.saveFile('content', [], []);
      });
      expect(res!).toBeNull();
    ...
    it('writes file and sidecar, returns the path', async () => {
      ...
      await act(async () => {
        savedPath = await result.current.saveFile('updated content', [], []);
      });
    ...
    it('uses forcePath when provided, overriding stored filePath', async () => {
      ...
      let savedPath: string | null;
      await act(async () => {
        savedPath = await result.current.saveFile('content', [], [], '/override/path.md');
      });
    ...
    it('deletes sidecar when comments and suggestions are both empty', async () => {
      ...
      const { result } = renderHook(() => useFileManager());
      await act(async () => {
        await result.current.saveFile('content', [], [], '/docs/test.md');
      });
    ...
    it('writes sidecar JSON when there are comments', async () => {
      ...
      const { result } = renderHook(() => useFileManager());
      await act(async () => {
        await result.current.saveFile('content', [SAMPLE_COMMENT], [], '/docs/test.md');
      });
  ...
  describe('saveFileAs', () => {
    it('appends .md when the dialog returns a path without it', async () => {
      ...
      let savedPath: string | null;
      await act(async () => {
        savedPath = await result.current.saveFileAs('content', [], []);
      });
