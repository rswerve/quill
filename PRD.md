...
- **Model / effort controls.** The footer carries two selects — **model** (AUTO, or `fable` / `opus` / `sonnet` / `haiku`) and **effort** (AUTO, or `low` / `medium` / `high` / `xhigh` / `max`) — read at spawn time and persisted globally (`quill-claude-model`, `quill-claude-effort`). **AUTO** lets Claude Code decide and omits the flag entirely. These set the next request, and the same selects also **report what Claude actually used** on the last run: in AUTO mode the control shows the observed value bare — the model **family** only (`OPUS`, with the version and any context-window tag like `[1m]` stripped) and the **effective effort** (`HIGH`), each observed from the live Claude Code run (the model from the stream's init event, the effort from a per-run signal). Until a value has been observed the control reads `AUTO`; an explicit pick shows that pick. The tooltip spells out the distinction — "Model: Auto — last observed OPUS" versus "Model: OPUS (chosen for the next request)" — and each reply card still carries the model that produced it.
...
Live **filename**, **word count**, **character count**, **line/column**, a **zoom** control, the **reference-folder** control (folder icon + "REFERENCE FOLDER"), the **model / effort** selects (each showing the effective model family and effort Claude last used, with the full picture in their tooltip), and the **✦ session** link control.
