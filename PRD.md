...
- Six selectable color **themes** (Paper, Sage, Mocha · Dragonfly, Watery · Adirondack, Rodeo · Ecological, and Gruvbox), persisted to local storage across launches.
...
- The prompt Claude receives includes the **highlighted anchor text** (as framing — what the user is commenting on), the **comment thread so far**, the **full current document**, and a listing of the **pending suggestions** already awaiting review (so Claude doesn't re-propose or conflict with them).
...
- **Claude can write edits directly into the document as tracked changes.** When the user asks for a revision (e.g. "tighten this", "fix the grammar"), Claude's reply carries a fenced `quill-edits` block of `find` / `replace` edits alongside its prose. Quill locates each `find` string and applies the replacement as a **tracked change attributed to Claude** — so AI revisions land as ordinary Accept / Reject suggestion cards in the margin, reviewed exactly like a human's. The prose explanation still appears in the thread; only the editing instructions are stripped from it.
  - **Edits are document-scale:** the highlight frames the request but does not fence it — Claude's edits may land anywhere in the document the request warrants (the prompt instructs it to keep changes minimal and relevant, with no unrequested rewrites elsewhere).
  - **Provenance:** every tracked change Claude makes from a comment is stamped with that comment's id (persisted in the sidecar). Its suggestion card carries a muted **"↳ comment" chip** that jumps to the originating comment, and while that comment is the active annotation its child suggestion cards get a subtle outline. A deleted comment degrades gracefully (no chip, no outline).
  - Edits whose `find` text can no longer be located in the document are skipped rather than misapplied, and the count is reported. Text-identical edits (`find` = `replace` — a formatting-only ask, which find/replace can't express) are skipped the same way, and the prompt tells Claude to answer formatting requests in prose instead.
...
- **Compaction-aware wording:** every request sends the **full current document**; before asking, Quill checks whether the linked session's context was compacted only to pick the accompanying note (context intact → "may have been edited since you wrote it"; compacted → an explicit compaction note). Skipped for Quill-created sessions, which never authored the doc.
