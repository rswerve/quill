...
- **`App.tsx`** — Orchestrates everything: editor ref, comment/suggestion state, keyboard shortcuts (Cmd+S/O/N, Cmd+Shift+S, Cmd+F), file metadata. (Link editing lives in `Toolbar.tsx`'s `LinkButton`, not App: clicking an existing link, pressing Cmd+K with selected text, or using the rail button opens one body-portaled editor for display text + URL, with Apply/Remove/Open actions. It captures the full target range before its inputs steal focus; shared primitives in `utils/linkEditing.ts` normalize safe hrefs, replace display text while retaining the link mark, and unlink without deleting text. Relative/fragment hrefs remain editable but cannot Open externally. Link and code mark changes pass through suggesting mode untracked; actual display-text replacements remain text edits — see `TrackChanges.ts`.)
