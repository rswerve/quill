...
- **`utils/typography.ts`** + `Toolbar.tsx`'s `FontControls` — the document **Font/Size** selectors (toolbar, left of the mode toggle). Font stamps `--font-doc-body` / `--font-doc-heading` inline on `:root` (headings keep a serif-contrast partner face by decision); Size stamps `--text-doc-base` (body px ÷ 1.125 — the body is 1.125em of the base, so all document ratios scale and zoom multiplies independently). Both persist in localStorage like the theme. Faces are bundled `@fontsource-variable` Mulish/Petrona (imported in `main.tsx`; the Google Fonts CDN link was deliberately removed from `index.html` — never reintroduce a font CDN) plus zero-byte `system-ui`/`ui-serif`. UI chrome stays on the `--font-sans`/`--font-serif` tokens and must not follow the document selection; `src/test/styles/documentFonts.test.ts` guards the wiring.
